...
class PWCNewsSpider(scrapy.Spider):
    ...
    async def parse(self, response):
        ...
        # Keep clicking the "Load More" button if it exists and load new content
        retries = 0
                ...
                if load_more_button:
                    ...
                    yield response.follow(
                        ...
                        meta={
                            "playwright": True,
                            "playwright_actions": [
                                PageMethod("click", "button.collection__load-more.ng-binding.ng-scope.primary")
                            ]
                        },
                    )
                    # Wait for the new content to load
                    time.sleep(2)  # Adjust as needed to give time for content to load
                    return  # Return to wait for the next set of content before continuing
                else:
